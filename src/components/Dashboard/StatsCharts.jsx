...
const StatsCharts = ({ 
    ...
    const [isGeoChartMaximized] = useState(false);
    ...
    useEffect(() => {
        if (geoData?.data) {
            ...
            const processed = aggregatedByLocation.map(item => {
                ...
                const hasCoordinates = item.latitude && item.longitude && item.latitude !== 0 && item.longitude !== 0;
    ...
    }, [stats, dateRange.dateFrom, dateRange.dateTo]);
                ...
                <Grid container>
                    <Grid item xs={12} sx={{width: '100%', gap: 2, display: 'flex', flexDirection: 'column'}}>
                        ...
                        <Box sx={{display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 1, mb: 1}}>
                            ...
                            <Typography className="last-updated" variant="caption">
                                {isRealTimeActive 
                        ...
                        <Box sx={{display: 'flex', flexWrap: 'wrap', gap: 1, '& > *': {flex: '1 1 0px', minWidth: {xs: 'calc(50% - 8px)', sm: 'calc(33% - 8px)', md: 'calc(20% - 8px)'}}}}>
                            <StatsCard title="dashboard.statistics.total_users" value={stats?.totalUsers || 0} icon={<UsersIcon fontSize="small"/>}/>
                            <StatsCard title="dashboard.statistics.total_posts" value={stats?.totalPosts || 0} icon={<PostsIcon fontSize="small"/>}/>
                            <StatsCard title="dashboard.statistics.total_comments" value={stats?.totalComments || 0} icon={<CommentsIcon fontSize="small"/>}/>
                            <StatsCard title="dashboard.statistics.total_likes" value={stats?.totalLikes || 0} icon={<LikesIcon fontSize="small"/>}/>
                            <StatsCard title="dashboard.statistics.total_shares" value={stats?.totalShares || 0} icon={<SharesIcon fontSize="small"/>}/>
                        </Box>
