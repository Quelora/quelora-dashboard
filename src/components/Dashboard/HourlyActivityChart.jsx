// ./src/components/Dashboard/HourlyActivityChart.jsx
import { Box, Paper, Typography, Stack, Tooltip } from '@mui/material';
import React from 'react'; // CORRECCIÓN: Importación de React
import {
    BarChart,
    Bar,
    XAxis,
...
const HourlyActivityChart = ({ chartData, timeBreakdown, dateRange, colors, t }) => {
    const timeIcons = {
        ...
        >
            <Paper elevation={0} sx={{
                ...
                <Typography variant="subtitle1" fontWeight={500} gutterBottom>
                    {t('dashboard.statistics.comments_replies')}
                </Typography>
                <Box sx={{
                    flex: 1,
                    minHeight: 300,
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={chartData} margin={{top: 10, right: 10, left: 0, bottom: 0}}>
                            ...
                            <Bar
                                ...
                                fillOpacity={0.8}
                                radius={[4, 4, 0, 0]}
                                name={t('dashboard.statistics.comments')}
                            />
                            <Bar
                                dataKey="replies"
                                fill={colors.replies}
                                fillOpacity={0.8}
                                radius={[4, 4, 0, 0]}
                                name={t('dashboard.statistics.replies')}
                            />
                        </BarChart>
                    </ResponsiveContainer>
                </Box>
                            ...
                            <Tooltip
                                ...
                                title={
                                    <React.Fragment>
                                        ...
                                        <Typography variant="caption" display="block">
                                            {t('dashboard.statistics.comments')}: {data.comments}%
                                        </Typography>
                                        <Typography variant="caption" display="block">
                                            {t('dashboard.statistics.replies')}: {data.replies}%
                                        </Typography>
                                    </React.Fragment>
                                }
                                arrow
