...
// ./src/components/Dashboard/PostTimeChart.jsx
...
const PostTimeChart = ({ analyzedPostsData = {}, postTitlesMap = {}, colors = ['#4285F4', '#DB4437', '#F4B400'], loading = false, dateRange, chartMetric, onMetricChange }) => {
    ...
    // CORRECCIÓN CLAVE 1: Fuerza la lectura de offsetWidth al montar
    useEffect(() => {
        // Ejecuta inmediatamente en el montaje
        const updateWidth = () => {
            if (containerRef.current) {
                const newWidth = containerRef.current.offsetWidth;
                if (newWidth > 0 && dimensions.width !== newWidth) {
                    setDimensions(prev => ({...prev, width: newWidth}));
                }
            }
        };

        // Usa setTimeout para asegurar que la lectura ocurra después de que el contenedor de TabPanel se haga visible
        const timeout = setTimeout(updateWidth, 100); 

        return () => clearTimeout(timeout);
    }, [containerRef]); // No incluir dimensions.width como dependencia, ya que solo queremos que corra al montar/remontar

    // Lógica de ResizeObserver (Mantenida)
    useEffect(() => {
    ...
    const combinedData = useMemo(() => {
        ...
        return data;
    }, [analyzedPostsData, postTitlesMap, colors, t, chartMetric]);
    ...
    if (loading && selectedCount > 0) {
        ...
        // Usamos dimensions.width que ahora será el ancho real
        const chartWidth = dimensions.width || containerRef.current?.offsetWidth || 1000;
