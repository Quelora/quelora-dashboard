...
const GeoDistributionChart = ({ processedGeoData, colors, t }) => {
  ...
  // Efecto para el enfoque automático inicial
  useEffect(() => {
    if (processedGeoData.length > 0 && !initialFocusDone && mapReady) {
      ...
      }, 1500); // Retraso para asegurar la carga completa
  ...
  // Centrar y hacer zoom en un elemento
  const focusOnItem = (item) => {
    if (!mapReady || !mapRef.current) return;
    
    const map = mapRef.current;
    const targetZoom = item.isRegion ? 6 : 10;
    const targetCenter = [item.lat, item.lng];
  ...
  const MapController = () => {
    ...
    useEffect(() => {
      ...
      const updateView = () => {
        ...
        setZoom(currentZoom);
        setCurrentView(currentZoom >= 8 ? 'city' : 'region');
      };
  ...
  // Calcular el radio de las burbujas
  ...
  return (
    <Box sx={{ flex: '1 1 calc(33.333% - 24px)', minWidth: 300, height: '100%', backgroundColor: 'var(--white)' }}>
      <Paper elevation={0} sx={{
          ...
          {processedGeoData.length === 0 ? (
            ...
            >
              ...
              {regionData.map((region, index) => {
                ...
                return (
                  <Circle
                    ...
                    radius={calculateRadius(region.total, true)}
                    ...
                    weight={style.weight}
                    eventHandlers={{
                      click: () => focusOnItem(region)
                    }}
                  >
                    <Popup>{region.region} - Total: {region.total}</Popup>
                  </Circle>
                );
              })}
              ...
              {processedGeoData.map((city, index) => {
                ...
                const cityWithRegionFlag = { ...city, isRegion: false };
                
                return (
                  <Circle
                    key={`city-${index}`}
                    center={[parseFloat(city.lat), parseFloat(city.lng)]}
                    radius={calculateRadius(city.total)}
                    fillOpacity={style.fillOpacity}
                    color={style.color}
                    fillColor={colors.default}
                    stroke
                    weight={style.weight}
                    eventHandlers={{
                      click: () => focusOnItem(cityWithRegionFlag)
                    }}
                  >
                    <Popup>{city.city}, {city.region} - Total: {city.total}</Popup>
                  </Circle>
                );
              })}
            </MapContainer>
        ...
        <Box sx={{ mt: 2 }}>
          ...
          <Stack direction="column" spacing={1}>
            {(currentView === 'region' ? regionData : processedGeoData)
              .sort((a, b) => b.total - a.total)
              .slice(0, 10)
              .map((item, index) => {
                const itemToUse = currentView === 'region' ? item : { ...item, isRegion: false };
                ...
                return (
                  <Box 
                    ...
                    sx={{
                      ...
                      p: 1,
                      borderRadius: 1,
                      bgcolor: highlightedItem === item ? 'action.selected' : 'background.paper',
                      '&:hover': {
                        bgcolor: 'action.hover',
                        cursor: 'pointer'
                      }
                    }}
                    onClick={() => focusOnItem(itemToUse)}
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <Place fontSize="small" sx={{ mr: 1, color: 'text.secondary' }} />
                      <Typography variant="caption">
                        {currentView === 'region' ? item.region : `${item.city}, ${item.region}`}
                      </Typography>
                    </Box>
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
