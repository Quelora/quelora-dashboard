...
import {
    ...
    VpnKey as ConnectionStringIcon,
 ...
 * @param {Function} props.handleModulesConfig     - Opens modules config modal.
...
const ClientCard = ({
    ...
    const [connStrModalOpen, setConnStrModalOpen] = useState(false);
    ...
    return (
        ...
        >
            ...
            <CardActions sx={{ flexDirection: 'column', gap: 1, px: 2.25, py: 1.5, alignItems: 'stretch' }}>
                ...
                <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 0.75 }}>
                        <Tooltip title={t('client.tooltip.connection_string')}>
                            <ActionIconButton onClick={() => setConnStrModalOpen(true)} size="small">
                                <ConnectionStringIcon sx={{ fontSize: '0.9rem' }} />
                            </ActionIconButton>
                        </Tooltip>
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
                    ...
                    <Tooltip title={t('client.tooltip.modules_config', 'Manage enterprise modules and community plugins')}>
                        <ModuleChip
                            chipState={
                                (client.enterpriseModules?.length || client.communityPlugins?.length)
                                    ? 'active'
                                    : 'default'
                            }
                            onClick={() => handleModulesConfig(client)}
                        >
                            <ExtensionIcon sx={{ fontSize: 12 }} />
                            {t('client.module_modules', 'Modules')}
                        </ModuleChip>
                    </Tooltip>
                </Box>
            </CardActions>
            ...
            <ConnectionStringModal
                open={connStrModalOpen}
                setOpen={setConnStrModalOpen}
                client={client}
                showToast={showToast}
            />
