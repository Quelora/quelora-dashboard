...
const EmailConfigModal = ({
    ...
    useEffect(() => {
        if (open && initialData?.email) {
            setEmailConfig({
                ...
                requires_auth: initialData.email.requires_auth !== false
            });
            setErrors({});
        }
    }, [open, initialData]);
    ...
    const handleEmailConfigChange = (field) => (event) => {
        const value = event.target.type === 'checkbox' ? event.target.checked : event.target.value.slice(0, maxLengths[field]);
        setEmailConfig((prev) => {
            const newState = {
                ...prev,
                [field]: value
            };
            // If auth is disabled, we clear the errors related to user/pass immediately
            if (field === 'requires_auth' && !value) {
                setErrors((prevErrors) => {
                    const newErrors = { ...prevErrors };
                    delete newErrors.smtp_user;
                    delete newErrors.smtp_pass;
                    return newErrors;
                });
            }
            return newState;
    ...
    const handleSave = async () => {
        if (!validateEmailConfig()) {
            showToast(t('client.email_config_incomplete'), 'warning');
            return;
        }
        ...
        try {
            ...
            if (saveResult) {
                ...
                showToast(t('client.email_config_saved'), 'success');
                if (!keepOpenOnSave) {
                    onClose();
                }
            }
        } catch (err) {
            showToast(t('client.email_config_save_error'), 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleTestEmail = async () => {
        ...
        try {
            ...
            if (saveResult) {
                setEmailConfig({ ...emailConfig });
                showToast(t('client.config_saved_before_test'), 'success');

                setOpenEmailConfigModal(false);

                // If auth is required, suggest the smtp user, otherwise leave empty
                const suggestedEmail = emailConfig.requires_auth ? emailConfig.smtp_user : '';
            ...
            showToast(t('client.test_email_error'), 'error');
        } finally {
            setLoading(false);
        }
    };
            ...
            <DialogContent>
                <Box sx={{ mt: 2 }}>
                    <CustomTextField
                        ...
                        margin="normal"
                        required
                        error={isFormSubmitted && (!!errors.smtp_host || (!emailConfig.smtp_host || emailConfig.smtp_host.length < 3))}
                        helperText={
                            errors.smtp_host ||
                    ...
                    <CustomTextField
                        ...
                        type="number"
                        required
                        error={isFormSubmitted && (!!errors.smtp_port || (!emailConfig.smtp_port || !isValidPort(emailConfig.smtp_port)))}
                        helperText={
                            errors.smtp_port ||
                    ...
                    <Box sx={{ mt: 2, mb: 1 }}>
                        <FormControlLabel
                            control={
                                <Switch
                                    checked={emailConfig.requires_auth}
                                    onChange={handleEmailConfigChange('requires_auth')}
                                    color="primary"
                                />
                            }
                            label={t('client.requires_authentication') || "Requires Authentication"}
                        />
                    </Box>

                    {emailConfig.requires_auth && (
                        <>
                            <CustomTextField
                                ...
                                margin="normal"
                                required
                                error={isFormSubmitted && (!!errors.smtp_user || (!emailConfig.smtp_user || emailConfig.smtp_user.length < 3))}
                                helperText={
                                    errors.smtp_user ||
                            ...
                            <CustomTextField
                                ...
                                required
                                error={isFormSubmitted && (!!errors.smtp_pass || (!emailConfig.smtp_pass || emailConfig.smtp_pass.length < 6))}
                                helperText={
                                    errors.smtp_pass ||
