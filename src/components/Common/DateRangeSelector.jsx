...
const isTodayRange = (from, to) => {
    ...
    return from.getTime() === startOfToday.getTime() && to.getTime() === endOfDay.getTime();
};
...
const DateRangeSelector = ({ 
    ...
    const [isInternalRealTimeActive, setIsInternalRealTimeActive] = useState(isRealTimeActive);


    const dispatchChange = useCallback((from, to) => {
        if (onRangeChange) {
    ...
    useEffect(() => {
        ...
        setIsInternalRealTimeActive(isRealTimeActive); 
    }, [propDateFrom, propDateTo, isRealTimeActive]);

    
    const handleRealTimeSwitch = (event) => {
        const isActive = event.target.checked;
        setIsInternalRealTimeActive(isActive);
        
        if (onRealTimeToggle) {
            onRealTimeToggle(isActive);
        }
    ...
    const handleQuickRangeSelect = (range) => {
        if (isInternalRealTimeActive && onRealTimeToggle) {
            onRealTimeToggle(false); 
        }
        setIsInternalRealTimeActive(false);
    ...
    const handleDateFromChange = (newValue) => {
        if (isInternalRealTimeActive && onRealTimeToggle) onRealTimeToggle(false);
        setIsInternalRealTimeActive(false);
        setActiveButton(null); 
    ...
    return (
        <Box sx={{ 
            ...
            <FormControlLabel
                control={
                    <Switch 
                        checked={isInternalRealTimeActive} 
                        onChange={handleRealTimeSwitch} 
                        color="primary"
                    />
                }
                label={t('common.realtime') || "Real-Time (5s)"}
                sx={{ 
                    mr: {sm: 2}, 
                ...
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                    ...
                    >
                        <DatePicker
                            ...
                            slotProps={{ 
                                textField: { 
                                 ...
                                 disabled: isInternalRealTimeActive
                                } 
                            }}
                        />
                        <DatePicker
                        ...
                        <Button 
                            ...
                            disabled={!dateFrom || !dateTo || isInternalRealTimeActive}
                            sx={{ 
                                minWidth: {xs: '100%', sm: 80},
                                mt: {xs: 1, sm: 0}
                            }}
                        >{t('common.rangedate.apply')}</Button>
                    </Stack>
